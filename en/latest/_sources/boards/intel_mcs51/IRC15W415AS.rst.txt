..  Copyright (c) 2014-present PlatformIO <contact@platformio.org>
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
       http://www.apache.org/licenses/LICENSE-2.0
    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

.. _board_intel_mcs51_IRC15W415AS:

Generic IRC15W415AS
===================

.. contents::

Hardware
--------

Platform :ref:`platform_intel_mcs51`: The Intel MCS-51 (commonly termed 8051) is an internally Harvard architecture, complex instruction set computer (CISC) instruction set, single chip microcontroller (uC) series developed by Intel in 1980 for use in embedded systems.

.. list-table::

  * - **Microcontroller**
    - IRC15W415AS
  * - **Frequency**
    - 11MHz
  * - **Flash**
    - 15.50KB
  * - **RAM**
    - 512B
  * - **Vendor**
    - `STC <https://www.stcmicro.com/STC/STC15W408AS.html?utm_source=platformio.org&utm_medium=docs>`__


Configuration
-------------

Please use ``IRC15W415AS`` ID for :ref:`projectconf_env_board` option in :ref:`projectconf`:

.. code-block:: ini

  [env:IRC15W415AS]
  platform = intel_mcs51
  board = IRC15W415AS

You can override default Generic IRC15W415AS settings per build environment using
``board_***`` option, where ``***`` is a JSON object path from
board manifest `IRC15W415AS.json <https://github.com/platformio/platform-intel_mcs51/blob/master/boards/IRC15W415AS.json>`_. For example,
``board_build.mcu``, ``board_build.f_cpu``, etc.

.. code-block:: ini

  [env:IRC15W415AS]
  platform = intel_mcs51
  board = IRC15W415AS

  ; change microcontroller
  board_build.mcu = irc15w415as

  ; change MCU frequency
  board_build.f_cpu = 11059200L

Debugging
---------
:ref:`piodebug` currently does not support Generic IRC15W415AS board.